...
;; Foreign function compatibility module for MCL, LispWorks, ACL 
...
(defun %new-array (type size)
  (fli:allocate-foreign-object :type (list :c-array type size)))
...
(defun %get-cstring (ptr &optional (start 0))
  (let ((size 0))
    ...
    (let ((str (make-string size)))
      (loop while (plusp size) ; fixed by Arthur Lemmens
            do 
...
(defun %get-string (ptr &key (start 0) end)
...
(defmacro %new-cstring (byte-count)
  `(fli:allocate-foreign-object :type :char :initial-element #\Null
				:nelems ,byte-count))
...
(fli:define-c-typedef string-ptr :pointer #+ignore (:pointer :char))
  ...
  (ecase format
    (:unsigned-byte-vector ; thanks for fix to Marc Battyani
     (let ((vector (make-array len :element-type '(unsigned-byte 8))))
       (dotimes (i len)
         (setf (aref vector i) (fli:dereference ptr :index i :type :unsigned-byte)))
       vector))
    (:bit-vector
     (let ((vector (make-array (ash len 3) :element-type 'bit)))
       (dotimes (i len)
